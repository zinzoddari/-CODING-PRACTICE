...
class Solution {
  public int solution(int n, int[] stations, int w) {
      ...
      for (int i = 1; i <= stations.length + 1; i++) {
          endIndex = i >= stations.length + 1 ? n + 1 : stations[i - 1] - (w);

          value += (endIndex - startIndex) / distance;
          value = (endIndex - startIndex) % distance > 0 ? value + 1 : value;

          startIndex = i >= stations.length + 1 ? 0 : stations[i - 1] + (w + 1);
      }
